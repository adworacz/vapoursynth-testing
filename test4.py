...
clip2 = core.std.BlankClip(clip=clip1, color=[113, 115, 115])
mask = core.std.BlankClip(clip=clip1, color=[235, 235,235])

with Timer() as target:
    if gpu:
        clip1 = core.std.TransferFrame(clip1, 1)
        clip2 = core.std.TransferFrame(clip2, 1)
        mask  = core.std.TransferFrame(mask, 1)
    ret = core.std.MaskedMerge(clips=[clip1, clip2], mask=mask)
    for i in range(num_extra_iterations):
        ret = core.std.MaskedMerge(clips=[ret, clip2], mask=mask)
